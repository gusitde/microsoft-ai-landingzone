...
locals {
  base_nsg_rules = {
    ...
    "appgw_rule01" = {
      ...
      access                       = "Allow"
      destination_address_prefixes = try(local.core_vnet_definition.subnets["AppGatewaySubnet"].address_prefix, null) != null ? [local.core_vnet_definition.subnets["AppGatewaySubnet"].address_prefix] : [cidrsubnet(local.core_vnet_definition.address_space, 4, 5)]
